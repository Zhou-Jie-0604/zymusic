...
<%
    ...
    String userBg = (user != null && user.getBackgroundUrl() != null) ? user.getBackgroundUrl() : null;
    int bgOpacity = (user != null) ? user.getBackgroundOpacity() : 80;
...
<body class="has-player-bar">
    ...
    <div class="bg-upload-container" style="position:fixed;top:70px;right:20px;z-index:1001;">
        <button class="bg-upload-btn" onclick="toggleBgUpload()" style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;border:none;padding:10px 20px;border-radius:25px;cursor:pointer;font-size:14px;">更换背景</button>
        <form class="bg-upload-form" id="bgUploadForm" action="updateProfile" method="post" enctype="multipart/form-data" style="display:none;margin-top:10px;">
            ...
            <input type="file" name="background" id="bgInput" accept="image/png,image/jpeg,.png,.jpg,.jpeg" onchange="this.form.submit()" style="display:none;">
        </form>
    </div>
    ...
    <script>
        ...
        function confirmBgOpacity() {
            ...
            document.getElementById('bgOpacityControl').classList.remove('active');
        ...
        function renderMusicComments(comments, musicId) {
            ...
            for (var i = 0; i < comments.length; i++) {
                ...
                if (c.parentId === 0) {
                    ...
                    if (window.currentUserId && window.currentUserId === c.userId) {
                        html += '<button class="delete-comment-btn" onclick="deleteMusicComment(' + c.id + ')" title="删除" style="background:none;border:none;color:#ff6b6b;cursor:pointer;font-size:13px;padding:0 2px;opacity:0.5;">×</button>';
                    }
                    html += '</div>';
                    ...
                    for (var j = 0; j < comments.length; j++) {
                        ...
                        if (nested.parentId === c.id) {
                            ...
                            if (window.currentUserId && window.currentUserId === nested.userId) {
                                html += '<button class="delete-comment-btn" onclick="deleteMusicComment(' + nested.id + ')" title="删除" style="background:none;border:none;color:#ff6b6b;cursor:pointer;font-size:13px;padding:0 2px;opacity:0.5;">×</button>';
                            }
                            html += '</div>';
                        }
                    }
        ...
        function deleteMusicComment(commentId) {
            ...
            .then(function(data) {
                if (data.success) {
                    loadMusicComments(window.currentMusicId);
                }
            })
            .catch(function(err) { console.error(err); });
        }
